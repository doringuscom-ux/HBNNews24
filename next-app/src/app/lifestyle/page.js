...
export async function generateMetadata() {
  try {
    ...
    const seo = await PageSeo.findOne({ pageUrl: '/lifestyle' });
...
export default async function Page() {
  ...
  try {
    ...
    const catNews = await News.find({ category: 'lifestyle' }).sort({ createdAt: -1 }).limit(15).lean();
    ...
    initialLatestNewsData = serializeNews(latestNews);
      ...
      console.error('Error fetching SSR data for lifestyle:', error);
  }

  return <Lifestyle initialNewsData={initialNewsData} initialLatestNewsData={initialLatestNewsData} />;
}
