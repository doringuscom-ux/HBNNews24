import Haryana from '@/views/Haryana';
...
export async function generateMetadata() {
  try {
    ...
    const seo = await PageSeo.findOne({ pageUrl: '/haryana' });
...
export default async function Page() {
  ...
  try {
    ...
    const catNews = await News.find({ category: 'haryana' }).sort({ createdAt: -1 }).limit(15).lean();
    ...
    initialLatestNewsData = serializeNews(latestNews);
      ...
      console.error('Error fetching SSR data for haryana:', error);
  }

  return <Haryana initialNewsData={initialNewsData} initialLatestNewsData={initialLatestNewsData} />;
}
