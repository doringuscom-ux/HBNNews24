...
export async function generateMetadata() {
  try {
    ...
    const seo = await PageSeo.findOne({ pageUrl: '/education' });
...
export default async function Page() {
  ...
  try {
    ...
    const catNews = await News.find({ category: 'education' }).sort({ createdAt: -1 }).limit(15).lean();
    ...
    initialLatestNewsData = serializeNews(latestNews);
      ...
      console.error('Error fetching SSR data for education:', error);
  }

  return <Education initialNewsData={initialNewsData} initialLatestNewsData={initialLatestNewsData} />;
}
