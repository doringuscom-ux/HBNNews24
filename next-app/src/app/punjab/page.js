import Punjab from '@/views/Punjab';
...
export async function generateMetadata() {
  try {
    ...
    const seo = await PageSeo.findOne({ pageUrl: '/punjab' });
...
export default async function Page() {
  ...
  try {
    ...
    const catNews = await News.find({ category: 'punjab' }).sort({ createdAt: -1 }).limit(15).lean();
    ...
    initialLatestNewsData = serializeNews(latestNews);
      ...
      console.error('Error fetching SSR data for punjab:', error);
  }

  return <Punjab initialNewsData={initialNewsData} initialLatestNewsData={initialLatestNewsData} />;
}
