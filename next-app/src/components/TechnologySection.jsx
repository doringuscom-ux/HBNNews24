...
export default function TechnologySection({ news = [] }) {
    const safeNews = (index, width = 300) => {
        ...
        return { image: "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 9'%3E%3Crect width='16' height='9' fill='%23e5e7eb'/%3E%3C/svg%3E", title: "Technology News", _id: "loading" };
    };
            ...
            <div className="w-full max-w-[1270px] mx-auto px-4">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="w-full md:w-[42%] flex flex-col md:pr-4 border-b md:border-b-0 md:border-r border-[#e5e5e5] pb-6 md:pb-0">
                        ...
                        {/* Top Main News */}
                        <Link href={mainNews._id !== 'loading' ? `/news/${mainNews._id}` : '#'} title={mainNews.title || "Technology News"} className="flex flex-col sm:flex-row gap-4 group cursor-pointer mb-5">
                            <div className="w-full sm:w-[230px] aspect-[16/9] overflow-hidden flex-shrink-0 bg-gray-50 rounded">
                                <img loading="lazy" width="400" height="250" src={optimizeImage(mainNews.image, 400)} alt={mainNews.title || "Technology News"} title={mainNews.title || "Technology News"} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" />
                            </div>
                        ...
                        {/* Bottom News Left */}
                        <Link href={bottomNewsLeft._id !== 'loading' ? `/news/${bottomNewsLeft._id}` : '#'} title={bottomNewsLeft.title || "Technology News"} className="border-t border-[#e5e5e5] pt-4 group cursor-pointer flex flex-col sm:flex-row gap-4">
                            <div className="w-full sm:w-[130px] aspect-[16/9] overflow-hidden flex-shrink-0 bg-gray-50 rounded">
                                <img loading="lazy" width="400" height="250" src={optimizeImage(bottomNewsLeft.image, 300)} alt={bottomNewsLeft.title || "Technology News"} title={bottomNewsLeft.title || "Technology News"} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" />
                            </div>
                    ...
                    <div className="w-full md:w-[29%] flex flex-col justify-between md:pr-4 border-b md:border-b-0 md:border-r border-[#e5e5e5] pb-6 md:pb-0">
                        {middleNewsList.map((news, index) => (
                            <Link href={news._id !== 'loading' ? `/news/${news._id}` : '#'} title={news.title || "Technology News"} key={index} className={`flex flex-col sm:flex-row gap-3 group cursor-pointer ${index !== middleNewsList.length - 1 ? 'border-b border-[#e5e5e5] pb-4 mb-4' : ''}`}>
                                <div className="w-full sm:w-[110px] aspect-[16/9] overflow-hidden flex-shrink-0 bg-gray-50 rounded">
                                    <img loading="lazy" width="400" height="250" src={optimizeImage(news.image, 300)} alt={news.title || "Technology News"} title={news.title || "Technology News"} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" />
                                </div>
                    ...
                    <div className="w-full md:w-[29%] flex flex-col justify-between">
                        {rightNewsList.map((news, index) => (
                            <Link href={news._id !== 'loading' ? `/news/${news._id}` : '#'} title={news.title || "Technology News"} key={index} className={`flex flex-col sm:flex-row gap-3 group cursor-pointer ${index !== rightNewsList.length - 1 ? 'border-b border-[#e5e5e5] pb-4 mb-4' : ''}`}>
                                <div className="w-full sm:w-[110px] aspect-[16/9] overflow-hidden flex-shrink-0 bg-gray-50 rounded">
                                    <img loading="lazy" width="400" height="250" src={optimizeImage(news.image, 300)} alt={news.title || "Technology News"} title={news.title || "Technology News"} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" />
                                </div>
                                <div className="flex-1 flex pt-1">
