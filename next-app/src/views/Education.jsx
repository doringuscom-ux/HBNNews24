...
export default function Education({ initialNewsData = [], initialLatestNewsData = [] }) {
            ...
            try {
                ...
                const catNews = allData.filter(item => {
                    if (Array.isArray(item.category)) {
                        return item.category.some(c => c.toLowerCase() === 'education' || c.toLowerCase() === 'शिक्षा' || c.toLowerCase() === 'एजुकेशन');
                    } else if (typeof item.category === 'string') {
                        return item.category.toLowerCase() === 'education' || item.category.toLowerCase() === 'शिक्षा' || item.category.toLowerCase() === 'एजुकेशन';
                    }
                    return false;
    ...
    return (
        <div className="w-full min-h-screen bg-[#f3f4f6]">
            {/* Banner Section */}
            <div className="w-full min-h-[120px] md:h-[160px] py-4 md:py-0 relative overflow-hidden flex items-center px-4 md:px-10" style={{ background: 'linear-gradient(to right, #1e3a8a, #2563eb, #60a5fa)' }}>
                
                {/* Background Watermark Right */}
                <div className="absolute right-0 top-1/2 transform -translate-y-1/2 pointer-events-none opacity-20 text-white hidden md:block">
                    <FaBuilding className="w-[200px] h-[200px]" />
                </div>

                {/* Left side Graphics */}
                <div className="absolute left-[20px] top-1/2 transform -translate-y-1/2 pointer-events-none opacity-20 text-white hidden md:block">
                    <BsGraphUpArrow className="w-[140px] h-[140px]" />
                </div>
                ...
                <div className="w-full max-w-[1270px] mx-auto flex flex-col md:flex-row items-start md:items-center justify-between relative z-10 md:pl-[120px] gap-4 md:gap-0">
                    ...
                    <div className="flex flex-col justify-center w-full md:w-auto">
                        <div className="flex items-center gap-2 mb-3 md:mb-1.5">
                            ...
                            <span className="text-white text-[13px] font-bold">/ एजुकेशन</span>
                        </div>
                        <div className="flex items-center justify-between w-full md:w-auto">
                            <div className="flex items-center gap-3">
                                {/* Mobile Icon */}
                                <BiBriefcase className="text-white w-10 h-10 md:hidden" />
                                {/* Desktop Icon */}
                                <div className="hidden md:flex bg-transparent items-center justify-center border-2 border-white rounded-full p-2">
                                    <BiBriefcase className="text-white w-[30px] h-[30px]" />
                                </div>
                                <h1 className="text-white text-[32px] md:text-[46px] font-black tracking-wide drop-shadow-md" style={{ fontFamily: '"Mukta", sans-serif' }}>एजुकेशन</h1>
                            </div>
