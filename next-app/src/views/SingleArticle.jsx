...
export default function SingleArticle({ initialArticle, initialLatestNews = [], initialRelatedNews = [] }) {
    const { id } = useParams();
    ...
    useEffect(() => {
        if (articleContentRef.current) {
            // Remove foreign font-family inline styles so uniform clean font applies, but preserve embeds
            try {
                const allElements = articleContentRef.current.querySelectorAll('*');
                allElements.forEach(el => {
                    if (el.closest('.twitter-tweet') || el.classList.contains('twitter-tweet')) return;
                    if (el.tagName === 'BLOCKQUOTE' || el.tagName === 'IFRAME') return;
    ...
    // Dedicated Twitter (X) widgets processor
    useEffect(() => {
        if (!articleContentRef.current) return;

        const hasTweets = articleContentRef.current.querySelector('.twitter-tweet');
        if (!hasTweets) return;
        ...
        if (!document.getElementById('twitter-wjs')) {
            ...
            loadWidgets();
            // Retry once after 500ms to guarantee widget renders after dynamic DOM updates
    ...
    return (
        <div className="w-full max-w-[1280px] mx-auto px-4 py-8 font-sans">
            ...
            {/* Bottom Full-Width Section: संबंधित खबरें (Related News) */}
            {relatedNews && relatedNews.length > 0 && (
                <div className="mt-14 pt-8 border-t-2 border-gray-200">
                    <div className="flex items-center justify-between pb-3 mb-6 border-b-2 border-[#da0000]">
                        <div className="flex items-center gap-2.5">
                            <span className="w-3 h-6 bg-[#da0000] rounded-[2px] inline-block"></span>
                            <h3 className="text-2xl font-black text-[#111] tracking-tight">संबंधित खबरें</h3>
                        </div>
                        {article?.category && (
                            <Link 
                                href={`/${(Array.isArray(article.category) ? article.category[0] : article.category).toLowerCase().replace(/\s+/g, '-')}`}
                                className="text-sm font-bold text-[#da0000] hover:underline flex items-center gap-1"
                            >
                                और खबरें &rarr;
                            </Link>
                        )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {relatedNews.slice(0, 8).map((news) => (
                            <Link 
                                ...
                                key={news._id}
                                className="group bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col hover:-translate-y-1"
                            >
                                <div className="relative aspect-[16/10] overflow-hidden bg-gray-100">
                                    <img 
                                        src={optimizeImage(news.image, 400) || "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 10'%3E%3Crect width='16' height='10' fill='%23e5e7eb'/%3E%3C/svg%3E"} 
                                        alt={news.title}
                                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                                    />
                                    {news.category && (
                                        <span className="absolute top-2.5 left-2.5 bg-[#da0000] text-white text-[11px] font-bold px-2 py-0.5 rounded shadow-md">
                                            {Array.isArray(news.category) ? news.category[0] : news.category}
                                        </span>
                                    )}
                                </div>
                                <div className="p-4 flex flex-col flex-1 justify-between">
                                    <h4 className="font-bold text-[15px] text-[#222] group-hover:text-[#da0000] transition-colors line-clamp-2 leading-[1.4] mb-3">
                                        {news.title}
                                    </h4>
                                    <span className="text-[12px] text-gray-500">
                                        {news.createdAt ? new Date(news.createdAt).toLocaleDateString('hi-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : ''}
                                    </span>
                                </div>
                            </Link>
                        ))}
