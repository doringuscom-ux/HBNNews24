        ...
        <div className="w-full min-h-screen bg-[#f3f4f6]">
            ...
            <div className="w-full min-h-[120px] md:h-[160px] py-4 md:py-0 relative overflow-hidden flex items-center px-4 md:px-10" style={{ background: 'linear-gradient(to right, #ef5a50, #fb9a5e)' }}>
                ...
                <div className="absolute right-0 top-1/2 transform -translate-y-1/2 pointer-events-none hidden md:block">
                    <img src={lifeImg?.src || lifeImg} alt="Life Graphic" className="h-[140px] w-auto object-contain drop-shadow-sm" />
                </div>
                ...
                <div className="absolute left-[-20px] top-1/2 transform -translate-y-1/2 pointer-events-none hidden md:flex items-center justify-center">
                    ...
                    <img src={cloudImg?.src || cloudImg} alt="Cloud" className="absolute w-[260px] h-auto object-contain z-0 opacity-80" style={{ WebkitMaskImage: 'radial-gradient(ellipse at center, rgba(0,0,0,1) 40%, rgba(0,0,0,0) 70%)', maskImage: 'radial-gradient(ellipse at center, rgba(0,0,0,1) 40%, rgba(0,0,0,0) 70%)' }} />
                    <img src={lifeStyleImg?.src || lifeStyleImg} alt="Lifestyle Graphic" className="h-[140px] w-auto object-contain relative z-10 drop-shadow-md" />
                </div>
                    ...
                    <div className="flex flex-col justify-center w-full md:w-auto">
                        ...
                        <div className="flex items-center justify-between w-full md:w-auto">
                            ...
                            <div className="flex items-center gap-3 text-white md:hidden">
                                ...
                                <FaXTwitter size={15} className="cursor-pointer hover:text-gray-300 drop-shadow-sm" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-8 flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                            ...
                            <div className="md:col-span-7 flex flex-col">
                                <Link href={`/news/${mainNews._id}`} className="group relative block overflow-hidden rounded-lg bg-gray-100">
                                ...
                                <Link href={`/news/${mainNews._id}`} className="mt-3">
                                    <h2 className="text-[20px] sm:text-[24px] font-extrabold text-[#111] leading-[1.3] hover:text-[#da0000] transition-colors line-clamp-2" style={{ fontFamily: '"Mukta", sans-serif' }}>
                                        {mainNews.title}
                                    </h2>
                                </Link>
                            ...
                            <div className="md:col-span-5 flex flex-col border-t md:border-t-0 md:border-l border-gray-100 pt-4 md:pt-0 md:pl-6">
                                <Link href={`/news/${topSideNews._id}`} className="group relative block overflow-hidden rounded-lg bg-gray-100">
                                ...
                                <Link href={`/news/${topSideNews._id}`} className="mt-3">
                                    <h3 className="text-[17px] font-bold text-[#111] leading-[1.35] hover:text-[#da0000] transition-colors line-clamp-3" style={{ fontFamily: '"Mukta", sans-serif' }}>
                                        {topSideNews.title}
                                    </h3>
                                </Link>
                        ...
                        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                            <div className="flex items-center justify-between border-b border-gray-200 pb-3 mb-4">
                                <div className="flex items-center gap-2">
                                    <span className="w-2.5 h-2.5 rounded-full bg-[#da0000]"></span>
                                    <h3 className="text-[18px] font-black text-[#111]" style={{ fontFamily: '"Mukta", sans-serif' }}>लेटेस्ट न्यूज़</h3>
                                </div>
                                <Link href="/" className="text-[12px] font-bold text-[#da0000] hover:underline">और देखें ›</Link>
                            </div>
                            ...
                            <div className="flex flex-col gap-4">
                                {latestNews.map((news, index) => (
                                    <Link href={`/news/${news.slug || news._id}`} key={news._id} className="flex gap-3 items-center group border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                                        <span className="text-[18px] font-extrabold text-gray-300 group-hover:text-[#da0000] transition-colors w-5 text-center flex-shrink-0">
                                            {index + 1}
                                        </span>
                                        ...
                                        <h4 className="text-[14px] font-bold text-[#222] leading-[1.3] group-hover:text-[#da0000] transition-colors line-clamp-2 flex-1" style={{ fontFamily: '"Mukta", sans-serif' }}>
                                            {news.title}
                                        </h4>
                                    </Link>
                                ))}
