...
export default function National({ initialNewsData = [], initialLatestNewsData = [] }) {
            ...
            try {
                ...
                const catNews = allData.filter(item => {
                    if (Array.isArray(item.category)) {
                        return item.category.some(c => c.toLowerCase() === 'national');
                    } else if (typeof item.category === 'string') {
                        return item.category.toLowerCase() === 'national';
                    }
                    return false;
    ...
    return (
        <div className="w-full min-h-screen bg-[#f3f4f6]">
            <div className="w-full min-h-[120px] md:h-[160px] py-4 md:py-0 relative overflow-hidden flex items-center px-4 md:px-10" style={{ background: 'linear-gradient(to right, #0369a1, #0284c7, #38bdf8)' }}>
                <div className="w-full max-w-[1270px] mx-auto flex flex-col md:flex-row items-start md:items-center justify-between relative z-10 md:pl-[120px] gap-4 md:gap-0">
                    <div className="flex flex-col justify-center w-full md:w-auto">
                        <div className="flex items-center gap-2 mb-3 md:mb-1.5">
                            ...
                            <span className="text-white text-[13px] font-bold">/ राष्ट्रीय</span>
                        </div>
                        <div className="flex items-center justify-between w-full md:w-auto">
                            <div className="flex items-center gap-3">
                                ...
                                <h1 className="text-white text-[32px] md:text-[46px] font-black tracking-wide drop-shadow-md" style={{ fontFamily: '"Mukta", sans-serif' }}>राष्ट्रीय</h1>
                            </div>
                        </div>
                    </div>
                </div>
